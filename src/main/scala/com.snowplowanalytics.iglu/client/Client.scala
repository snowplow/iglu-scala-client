...
import validator.{CirceValidator, Validator}
 ...
 */
case class Client[F[_], A](resolver: Resolver[F], validator: Validator[A]) {
...
object Client {
  ...
  val IgluCentral: Client[IO, Json] =
    Client[IO, Json](Resolver(None, List(Registry.IgluCentral)), CirceValidator)
}
