...
case class Resolver[F[_]](cache: Option[SchemaCache[F]], repos: List[Registry]) {
  import Resolver._
  ...
  def lookupSchema(schemaKey: SchemaKey, attempts: Int)(
    ...
    F: Monad[F],
    L: RegistryLookup[F],
    C: Clock[F]): F[Either[ResolverError, Json]] = {
    val prioritize = prioritizeRepos(schemaKey, _: RepositoryRefs).toList
    def retryCached(serverErrors: RepoFailuresMap): F[SchemaLookup] = {
...
object Resolver {
  ...
  def traverseRepos[F[_]: Monad: RegistryLookup](
    ...
    tried: RepoFailuresMap): F[SchemaLookup] = remaining match {
    case Nil => Applicative[F].pure(tried.asLeft)
    case repo :: tail =>
      def continueWith(error: LookupHistory): F[SchemaLookup] =
      ...
      RegistryLookup[F].lookup(repo, schemaKey).flatMap {
        ...
        case Left(e) => // TODO: we can pattern match here
          continueWith(LookupHistory(Set(e), 1, fatal = false))
      }
  }
   ...
   */
  def init[F[_]: Applicative: Cache](cacheSize: Int, refs: Registry*): F[Resolver[F]] =
    SchemaCache.init[F](cacheSize, None).map(cacheOpt => new Resolver(cacheOpt, List(refs: _*)))

  // Keep this up-to-date
  private[client] val EmbeddedSchemaCount = 4

  /** A Resolver which only looks at our embedded repo */
  def bootstrap[F[_]: Applicative: Cache]: F[Resolver[F]] =
   ...
   * @return a configured Resolver instance
   */
  def parse[F[_]: Monad: Cache](config: Json): F[Either[DecodingFailure, Resolver[F]]] = {
    ...
    } yield Resolver(cacheOpt, refsE)

    result.value
  }

  private def finish[F[_], A](result: A)(
    implicit F: Applicative[F]): F[Either[RepoFailuresMap, A]] =
    Applicative[F].pure(result.asRight[RepoFailuresMap])
  ...
  private def postProcess[F[_]](result: SchemaLookup) =
    result
      .leftMap(failure => failure.map { case (key, value) => (key.config.name, value) })
      .leftMap(map => ResolverError(map))
