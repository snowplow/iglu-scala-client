...
class ResolverCache[F[_]] private (
  ...
  schemaLists: LruMap[F, (String, String), SchemaList],
  val ttl: Option[Int]) {
  ...
  def getSchemaList(vendor: String, name: String)(implicit F: Monad[F]): F[Option[SchemaList]] =
    for {
      list <- schemaLists.get((vendor, name))
    } yield list
...
object ResolverCache {
  def init[F[_]: Monad](size: Int, ttl: Option[Int])(
    ...
    L: CreateLruMap[F, (String, String), SchemaList]): F[Option[ResolverCache[F]]] =
    Option(())
      .filter(_ => size > 0)
      ...
      case Some(_) =>
        for {
          ...
          schemaLists <- CreateLruMap[F, (String, String), SchemaList].create(size)
