...
class SchemaCriterionSpec extends Specification with DataTables with ValidationMatchers {
  ...
  def e1 = {
    ...
    SchemaCriterion("com.snowplowanalytics.snowplow", "payload_data", "jsonschema", 1).matches(
      actual) must beFalse
  }
    ...
    "SPEC NAME" || "Criterion" | "SchemaVer" | "EXPECTED OUTPUT" |
      ...
      (_, criterion, version, expected) =>
        SchemaCriterion(
          ...
          criterion._3)
          .matches(
            SchemaKey("com.snowplowanalytics.snowplow", "payload_data", "jsonschema", version))
          .must_==(expected)
    }
