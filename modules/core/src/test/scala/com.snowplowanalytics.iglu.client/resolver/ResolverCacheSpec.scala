...
class ResolverCacheSpec extends Specification {
  ...
  def e1 = {
    ...
    val lookupResult = (schema, None).asRight[LookupFailureMap]

    val expectedState =
      RegistryState(Map.empty, 4, List((key, (2, Right((Json.Null, None))))), 5, List())
    ...
    val schemaResult    = result must beRight((schema, None))
