...
class ResolverResultSpec extends Specification with DataTables with ValidatedMatchers {
  ...
  def e6 = {
    ...
    val thirdSucceeded = response3 must beRight[SchemaLookupResult].like {
      case ResolverResult.Cached(key, (value, _), _) =>
  ...
  def e12 = {
    ...
    result must beRight(ResolverResult.NotCached((expectedSchema, None)))
  }
  ...
  def e13 = {
    ...
    val result = for {
      resolver  <- Resolver.init[StaticLookup](10, Some(200), httpRep)
      response1 <- resolver.lookupSchemaResult(schemaKey)
      _         <- StaticLookup.addTime(150) // ttl 200, delay 150
      response2 <- resolver.lookupSchemaResult(schemaKey)
  ...
  def e14 = {
    ...
    val result = for {
      resolver  <- Resolver.init[StaticLookup](10, Some(200), httpRep)
      response1 <- resolver.lookupSchemaResult(schemaKey)
      _         <- StaticLookup.addTime(250) // ttl 200, delay 250
      response2 <- resolver.lookupSchemaResult(schemaKey)
    } yield (response1, response2)
  ...
  def e15 = {
    ...
    result must beRight(ResolverResult.NotCached((expectedSchema, Some(SchemaVer.Full(1, 0, 2)))))
  }
}
