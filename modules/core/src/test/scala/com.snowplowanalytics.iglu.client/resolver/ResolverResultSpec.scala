...
class ResolverResultSpec extends Specification with DataTables with ValidatedMatchers with CatsEffect {
  def is = s2"""
  ...
  def e2 = {
    ...
    } yield parsed must beRight[Resolver[IO]].like { case resolver =>
  ...
  def e3 = {
    ...
    SpecHelpers.TestResolver
      .flatMap(resolver =>
        resolver.lookupSchemaResult(schemaKey).map(_.leftMap(SpecHelpers.cleanTimestamps))
      )
  ...
  def e7 = {
    ...
    val result = for {
      ...
      _      <- StaticLookup.addTime(2.seconds)
      _      <- resolver.lookupSchemaResult(schemaKey)
      _      <- StaticLookup.addTime(2000.seconds)
