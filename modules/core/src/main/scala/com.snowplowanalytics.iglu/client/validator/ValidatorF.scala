...
trait ValidatorF[F[_], A] {
   ...
   */
  def validateF(data: A, schema: SchemaLookupResult): F[Either[ValidatorError, Unit]]
   ...
   */
  def checkSchemaF(schema: SchemaLookupResult): F[List[ValidatorError.SchemaIssue]]
   ...
   */
  def validateSchemaF(schema: SchemaLookupResult)(implicit
    F: Functor[F]
